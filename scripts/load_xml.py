...
def load_scene(filepath):
    ...
    for plugin in root:
        if plugin.tag == "default":
            ...
            elif plugin.attrib["name"] == "fov" and "fov" not in scene_params.keys():
                scene_params["fov"] = float(plugin.attrib["value"])
            ...
            for prop in plugin:
                ...
                if prop.tag == "float":                 
                    if prop.attrib["name"] == "near_clip" and "near_clip" not in scene_params.keys():

...
def get_start(mesh, top=True):
    ray_origins = np.array([[0, 0, 2],]) if top else np.array([[0, 0, -2],])
    ray_directions = np.array([[0, 0, -1]]) if top else np.array([[0, 0, 1],])
...
class Bridge:
    ...
    def __init__(self, path, offset=0.02, flip_mesh=False):
        self.path = path        
        self.mesh = scale_mesh(path, offset=offset)
        if flip_mesh:
            self.mesh = flip_mesh(self.mesh)
...
def sample_all_paths(paths, samples_num, concatenate_opposite=True):
    all_samples = []
    for path in paths:
